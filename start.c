...
static void InitPaths(void) {
    ...
    swprintf(g_appDataDir, MAX_PATH, L"%s\\%s", g_localAppData, APP_DIR_NAME);
    swprintf(g_destPath, MAX_PATH, L"%s\\extension", g_appDataDir);
    swprintf(g_userDataDir, MAX_PATH, L"%s\\edge-profile", g_appDataDir);
    wcscpy(g_edgePath, DEFAULT_EDGE_PATH);
    swprintf(g_installedBatPath, MAX_PATH, L"%s\\%s", g_appDataDir, BAT_FILE_NAME);
...
static void InstallApp(void) {
    ...
    swprintf(shortcutDesktop, MAX_PATH, L"%s\\%s.lnk", desktopPath, APP_NAME);
    swprintf(shortcutStart, MAX_PATH, L"%s\\Microsoft\\Windows\\Start Menu\\Programs\\%s\\%s.lnk", startMenuDir, APP_NAME, APP_NAME);
    swprintf(shortcutUninstall, MAX_PATH, L"%s\\Microsoft\\Windows\\Start Menu\\Programs\\%s\\卸载 %s.lnk", startMenuDir, APP_NAME, APP_NAME);
    
    WCHAR startMenuPath[MAX_PATH];
    swprintf(startMenuPath, MAX_PATH, L"%s\\Microsoft\\Windows\\Start Menu\\Programs\\%s", startMenuDir, APP_NAME);
...
static void UninstallApp(void) {
    ...
    swprintf(cmd, MAX_PATH * 2, L"rd /s /q \"%s\"", startMenuPath);
    _wsystem(cmd);
    wprintf(L"已删除开始菜单\n");
    ...
    swprintf(cmd, MAX_PATH * 2, L"rd /s /q \"%s\"", g_appDataDir);
    _wsystem(cmd);
...
static void StartEdge(void) {
    ...
    swprintf(args, MAX_PATH * 4, 
        L"\"%s\" --load-extension=\"%s\" --user-data-dir=\"%s\" --no-first-run --no-default-browser-check %s %s",
    ...
    if (CreateProcessW(NULL, args, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
        ...
        PrintError(L"无法启动 Edge: %s", g_edgePath);
    }
}
...
static void MainLogic(void) {
    ...
    swprintf(localManifestPath, MAX_PATH, L"%s\\manifest.json", g_destPath);
    ...
    if (needDownload) {
        ...
        swprintf(testPath, MAX_PATH, L"%s\\dist\\extension\\manifest.json", extractPath);
        if (GetFileAttributesW(testPath) != INVALID_FILE_ATTRIBUTES) {
            swprintf(srcPath, MAX_PATH, L"%s\\dist\\extension", extractPath);
        } else {
            swprintf(testPath, MAX_PATH, L"%s\\extension\\manifest.json", extractPath);
            if (GetFileAttributesW(testPath) != INVALID_FILE_ATTRIBUTES) {
                swprintf(srcPath, MAX_PATH, L"%s\\extension", extractPath);
            }
        }
        ...
        swprintf(copyCmd, MAX_PATH * 4, L"xcopy /E /Y /I \"%s\\*\" \"%s\"", srcPath, g_destPath);
        _wsystem(copyCmd);
        
        DeleteFileW(zipPath);
        swprintf(copyCmd, MAX_PATH * 4, L"rd /s /q \"%s\"", extractPath);
        _wsystem(copyCmd);
