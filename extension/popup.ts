...
const getImageSrc = () => imageSrc.at(currentImageIndex) || imageUrl;
function loadAISettings() {
  ...
  chrome.storage.local.get(
    ["aiModel", "aiPrompt", "apiKeys", "imageSrc"],
    (result) => {
      ...
      modelSelect.value =
        (settings.aiModel as string) || defaultAISettings.model;
      promptTextarea.value =
        (settings.aiPrompt as string) || defaultAISettings.prompt;
      console.log(promptTextarea.value);
      if (apiKeys?.doubaoKey) {
        doubaoKeyInput.value = apiKeys.doubaoKey;
      }
      if ((settings.imageSrc as string[])?.length) {
        imageSrc = settings.imageSrc as string[];
      }
      updateImages();
...
function mark() {
  recognizeImage(cardImage.src).then(
    (res) => {
      console.log("AI recognition result:", res);
      result.innerText = parseAIResult(res);
      result.style.color = "black";
    },
    (error) => {
      console.error("AI recognition failed:", error);
      result.innerText = `AI recognition failed: ${error}`;
...
// 提示词变化事件监听
promptTextarea.addEventListener("blur", saveAISettings);
// API Key变化事件监听
...
const imageHistoryInfo = document.getElementById(
  "imageHistoryInfo"
) as HTMLSpanElement;
...
const updateNavigationButtons = () => {
  ...
  nextImageButton.disabled = currentImageIndex >= imageSrc.length - 1;
};
...
// 键盘快捷键监听，使用Ctrl+V粘贴图片
document.addEventListener("paste", pasteImageFromClipboard);

// 历史记录导航按钮事件
prevImageButton.addEventListener("click", () => showImage(-1));
nextImageButton.addEventListener("click", () => showImage(1));
